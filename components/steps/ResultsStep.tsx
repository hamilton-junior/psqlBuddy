   ...
   return (
      <div className="fixed inset-0 z-[60] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-200" onClick={onClose}>
         <div className="bg-white dark:bg-slate-800 w-full max-w-2xl max-h-[85vh] rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto p-0 bg-slate-50 dark:bg-slate-900 custom-scrollbar">{viewMode === 'table' ? (<table className="w-full text-left border-collapse"><tbody className="divide-y divide-slate-100 dark:divide-slate-800">{filteredEntries.map(([key, val]) => (<tr key={key} className="group hover:bg-white dark:hover:bg-slate-800 transition-colors"><td className="px-4 py-3 w-1/3 bg-slate-100/50 dark:bg-slate-900/50 text-xs font-bold text-slate-500 dark:text-slate-400 border-r border-slate-100 dark:border-slate-800 font-mono break-all">{key}</td><td className="px-4 py-3 text-sm text-slate-700 dark:text-slate-200 relative break-all whitespace-pre-wrap">{val === null ? <span className="text-slate-400 italic text-xs">null</span> : String(val)}</td></tr>))}</tbody></table>) : (<div className="p-4"><pre className="text-xs font-mono text-slate-600 dark:text-slate-300 whitespace-pre-wrap break-all p-4 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">{JSON.stringify(row || {}, null, 2)}</pre></div>)}</div>
         </div>
      </div>
   );
};
   ...
   const formatValue = (col: string, val: any, rowIdx: number) => {
      ...
      const renderContent = () => {
         if (isSensitive && !isRevealed) {
            return (
               <div className="flex items-center gap-2">
                  <span className="blur-sm select-none opacity-40 font-mono grayscale">{String(displayVal).replace(/./g, '*').substring(0, 15)}</span>
         ...
         if (links.length > 0 && displayVal !== '') {
            return (
               <div className="flex items-center gap-1">
                  <button onClick={(e) => { e.stopPropagation(); if (hoverTimeoutRef.current) clearTimeout(hoverTimeoutRef.current); if (links.length === 1) { setHoverPreview(null); onDrillDown(links[0].table, links[0].keyCol, displayVal, links); } else { setHoverPreview({ links, val: displayVal, x: e.clientX, y: e.clientY, persistent: true }); } }} onMouseEnter={(e) => { const x = e.clientX; const y = e.clientY; if (!hoverPreview?.persistent && links.some(l => l.previewCol)) { hoverTimeoutRef.current = setTimeout(() => { setHoverPreview({ links, val: displayVal, x, y, persistent: false }); }, 350); } }} onMouseLeave={() => { if (!hoverPreview?.persistent) { if (hoverTimeoutRef.current) clearTimeout(hoverTimeoutRef.current); setHoverPreview(null); } }} className={`hover:underline flex items-center gap-1 group/link text-left min-w-0 ${links.length > 1 ? 'text-purple-600 dark:text-purple-400' : 'text-indigo-600 dark:text-indigo-400'} ${isPending ? 'bg-orange-50 dark:bg-orange-900/20 px-1 rounded border border-orange-200' : ''}`}><span className="truncate">{highlightMatch(String(displayVal))}</span>{links.length > 1 ? <Layers className="w-3 h-3 shrink-0" /> : <ExternalLink className="w-3 h-3 opacity-0 group-hover/link:opacity-100 shrink-0" />}</button>
                  {revealToggle}
               </div>
            );
         }
         return (
            <div className="flex items-center gap-2 overflow-hidden">
               <span className={`truncate block ${isPending ? 'text-orange-600 dark:text-orange-400 font-bold' : ''}`}>{highlightMatch(String(displayVal))}</span>
...
const ExplainVisualizer = ({ plan, loading, error, onCaptureProfiling }: { plan: ExplainNode | null, loading: boolean, error: string | null, onCaptureProfiling: () => void }) => {
   if (loading) return (<div className="p-8 space-y-6"><div className="flex items-center gap-4"><Skeleton className="w-12 h-12 rounded-full" /><div className="space-y-2 flex-1"><Skeleton className="w-1/4 h-4" /><Skeleton className="w-1/3 h-3 opacity-50" /></div></div><div className="space-y-4 pl-8">{[1, 2, 3].map(i => (<div key={i} className="p-4 border border-slate-100 dark:border-slate-800 rounded-xl space-y-3"><Skeleton className="w-1/3 h-4" /><div className="flex gap-4"><Skeleton className="w-20 h-3" /><Skeleton className="w-20 h-3" /></div></div>))}</div></div>);
   ...
   const renderNode = (node: ExplainNode, depth: number = 0) => (<div key={Math.random()} style={{ marginLeft: depth * 20 }} className="mb-2 group"><div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded p-3 shadow-sm inline-block min-w-[300px] hover:border-indigo-400 transition-colors"><div className="flex justify-between font-bold text-xs text-slate-700 dark:text-slate-200"><span className="text-indigo-600 dark:text-indigo-400">{node.type}</span><span className="text-slate-400 bg-slate-100 dark:bg-slate-900 px-2 rounded-full">{(node.cost.total || 0).toFixed(2)} cost</span></div>{node.relation && <div className="text-[11px] text-slate-600 dark:text-slate-300 mt-1 font-mono font-bold">{node.relation}</div>}<div className="text-[10px] text-slate-500 mt-2 flex gap-4 pt-2 border-t border-slate-100 dark:border-slate-700"><span className="flex items-center gap-1"><Hash className="w-3 h-3" /> Rows: {node.rows}</span><span className="flex items-center gap-1"><Database className="w-3 h-3" /> Width: {node.width}</span></div></div>{node.children && node.children.map(child => renderNode(child, depth + 1))}</div>);
   return (<div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900"><div className="p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-white dark:bg-slate-800"><div className="flex items-center gap-3"><div className="p-1.5 bg-amber-100 dark:bg-amber-900/30 rounded text-amber-600 dark:text-amber-400"><Gauge className="w-4 h-4" /></div><span className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase tracking-widest">Plano de Execução Detalhado</span></div><button onClick={onCaptureProfiling} className="flex items-center gap-2 px-4 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold shadow-md transition-all active:scale-95"><Camera className="w-3.5 h-3.5" /> Capturar Profiling Snapshot</button></div><div className="flex-1 overflow-auto p-6">{renderNode(plan)}</div></div>);
};
...
const ResultsStep: React.FC<ResultsStepProps> = ({ data, sql, onBackToBuilder, onNewConnection, settings, onShowToast, credentials, executionDuration, schema, resultsState, onResultsStateChange }) => {
  ...
  const ansiTableString = useMemo(() => { if (filteredData.length === 0) return "Nenhum dado retornado para os filtros atuais."; const cols = Object.keys(filteredData[0]); const colWidths = cols.map(col => Math.max(col.length, ...filteredData.map(row => { const v = row[col]; return v === null ? 4 : String(v).length; }))); const reset = "\x1b[0m"; const green = "\x1b[32m"; const blue = "\x1b[34m"; const yellow = "\x1b[33m"; const red = "\x1b[31m"; const cyan = "\x1b[36m"; let out = ""; out += "┌" + colWidths.map(w => "─".repeat(w + 2)).join("┬") + "┐\n"; out += "│ " + cols.map((col, i) => `${blue}${col.padEnd(colWidths[i])}${reset}`).join(" │ ") + " │\n"; out += "├" + colWidths.map(w => "─".repeat(w + 2)).join("┼") + "┤\n"; filteredData.forEach(row => { out += "│ " + cols.map((col, i) => { const val = row[col]; let str = val === null ? 'NULL' : String(val); let color = ""; if (val === null) color = red; else if (typeof val === 'boolean') color = val ? green : red; else if (typeof val === 'number') color = yellow; else if (col.toLowerCase().includes('id') || col.toLowerCase() === 'grid') color = cyan; return `${color}${str.padEnd(colWidths[i])}${reset}`; }).join(" │ ") + " │\n"; }); out += "└" + colWidths.map(w => "─".repeat(w + 2)).join("┴") + "┘"; return out; }, [filteredData]);
