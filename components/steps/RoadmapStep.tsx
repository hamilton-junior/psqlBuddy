...
import { 
  ...
  Clock, History, Dna, Play, Info, ThumbsUp
} from 'lucide-react';
...
const RoadmapStep: React.FC<{ onNavigate?: (step: AppStep) => void }> = ({ onNavigate }) => {
  ...
  const renderItemCard = (item: RoadmapItem, isTimeline: boolean = false) => {
      ...
      >
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-2 mt-2">
                   ...
                   {item.status === 'Finalizado' && (
                      <span className="animate-pulse flex items-center gap-1 text-[9px] font-black text-indigo-500 uppercase bg-indigo-50 dark:bg-indigo-950 px-2 py-1 rounded-full border border-indigo-100 dark:border-indigo-900">
        ...
        {isExpanded && (
            <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800 space-y-6 animate-in fade-in slide-in-from-top-1 duration-300">
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                 {item.status === 'Finalizado' && item.featureLink && onNavigate && (
                    <button 
                       onClick={(e) => { e.stopPropagation(); onNavigate(item.featureLink!); }}
                       className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl shadow-emerald-900/20 transition-all active:scale-95"
                    >
                       <Play className="w-4 h-4 fill-current" /> Experimentar Agora
                    </button>
                 )}
                 <button className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-white dark:bg-slate-800 text-slate-500 hover:text-indigo-600 border border-slate-200 dark:border-slate-700 font-black text-xs uppercase tracking-widest rounded-2xl transition-all">
                    <Info className="w-4 h-4" /> Ver Documentação
                 </button>
              </div>
            </div>
        )}
        ...
        {!isExpanded && (
            <div className="mt-6 pt-4 border-t border-slate-50 dark:border-slate-700/50 flex justify-between items-center">
              <span className="text-[10px] text-slate-400 font-bold uppercase flex items-center gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                <Clock className="w-3 h-3" /> Clique para detalhes técnicos
              </span>
              <button 
              ...
              >
                ...
                <span className={`ml-1 px-1.5 rounded-full ${hasVoted ? 'bg-emerald-200/50' : 'bg-indigo-200/50'}`}>
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-500 overflow-hidden">
      ...
      <div className="mb-6 flex flex-wrap items-center gap-2 shrink-0">
         {[
           { id: 'all', label: 'Todos os Itens', icon: <Layers className="w-3.5 h-3.5" /> },
                  ...
                  return (
                    <div key={idx} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 gap-6">
                          {filteredCatItems.map((item, iIdx) => renderItemCard(item))}
                        </div>
                    </div>
                  );
               })}
                  ...
                  {filteredItems.map((item, idx) => {
                     ...
                     return (
                        <div key={idx} className="relative flex flex-col md:flex-row items-center gap-10">
                           {/* Marker */}
                           <div className={`absolute left-8 md:left-1/2 w-6 h-6 rounded-full border-4 border-slate-50 dark:border-slate-900 -translate-x-1/2 z-10 transition-colors
                              ${item.status === 'Finalizado' ? 'bg-emerald-500' : item.status === 'Em Desenvolvimento' ? 'bg-orange-500' : 'bg-slate-400'}
                           `}></div>
                           ...
                           <div className={`w-full md:w-1/2 pl-16 md:pl-0 ${isEven ? 'md:text-right md:order-1' : 'md:text-left md:order-3'}`}>
                              <div className={`inline-flex flex-col ${isEven ? 'md:items-end' : 'md:items-start'}`}>
                                 <span className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.3em] mb-1">Versão Lançamento</span>
