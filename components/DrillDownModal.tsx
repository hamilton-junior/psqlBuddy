...
const DrillDownModal: React.FC<DrillDownModalProps> = ({ targetTable, filterColumn, filterValue, credentials, onClose, schema }) => {
  ...
  const saveUserMapping = (targetTable: string, column: string) => {
    ...
    // Salva a associação: Para esta tabela, quando eu clicar num link vindo de fora, buscar por ESTA coluna
  ...
  const fetchData = async (column: string, value: any, isManualMapping = false) => {
    ...
    try {
      ...
      // Forçamos cast para text para garantir que a comparação funcione independente do tipo da PK
      ...
      if (res.length === 1) {
        ...
        setError(`O valor "${value}" não foi encontrado na coluna "${column}". Escolha outro campo.`);
        setViewMode('map_column');
      }
    } catch (e: any) {
      setError(e.message);
      if (!isManualMapping) setViewMode('map_column');
    } finally {
      setLoading(false);
    }
  };
           ...
           {loading ? (
              ...
              <div className="p-8 max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
                 <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-6 mb-8 flex items-start gap-5 shadow-sm">
                    ...
                    <div>
                       <h4 className="font-bold text-amber-900 dark:text-amber-100 text-xl">Defina o campo de busca</h4>
                       <p className="text-amber-700 dark:text-amber-300 text-sm mt-1 leading-relaxed">
                          Não encontramos o valor <strong>"{filterValue}"</strong> nas colunas automáticas da tabela <strong>{targetTable}</strong>. 
                          Qual coluna devemos filtrar para trazer o registro correto?
                       </p>
                    </div>
                 </div>
                 ...
                 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    {filteredTargetColumns.length === 0 && (
                      <div className="col-span-full py-10 text-center text-slate-400 italic">Nenhuma coluna encontrada para "{columnFilterTerm}"</div>
                    )}
                 </div>
              </div>
           ) : viewMode === 'select' ? (
              <div className="p-8 max-w-3xl mx-auto">
                 <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-2xl p-6 mb-6 flex items-start gap-4">
                    ...
                    <div>
                       <h4 className="font-bold text-indigo-900 dark:text-indigo-100 text-lg">Qual registro você busca?</h4>
                       <p className="text-indigo-700 dark:text-indigo-300 text-sm mt-1">
                          Vários registros foram encontrados para <strong>"{filterValue}"</strong>.
                       </p>
                    </div>
                 </div>
                             ...
                             <div className="flex items-center gap-4">
                                ...
                                <div>
                                   <div className="font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                                      {String(row[descCol] || 'Sem descrição')}
                                   </div>
              ...
              <div className="p-12 text-center text-red-500 flex flex-col items-center">
                 ...
                 <button onClick={() => setViewMode('map_column')} className="mt-6 px-6 py-2 bg-indigo-600 text-white rounded-lg text-xs font-bold hover:bg-indigo-700 transition-colors">Escolher coluna manualmente</button>
              </div>
           ) : (
                       ...
                       {filteredData.map((row, rowIndex) => (
                          <div key={rowIndex} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-300">
                             <div className="px-4 py-2 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Resultado encontrado</span>
