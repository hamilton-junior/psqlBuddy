...
import { X, Database, ArrowRight, Loader2, AlertCircle, LayoutGrid, List, Search, Copy, Check, Filter, MousePointer2, ChevronRight, Info } from 'lucide-react';
...
interface DrillDownModalProps {
  ...
  onClose: () => void;
  schema?: DatabaseSchema; // Adicionado para detectar colunas de fallback
}

type ViewMode = 'table' | 'cards' | 'select';
...
const DrillDownModal: React.FC<DrillDownModalProps> = ({ targetTable, filterColumn, filterValue, credentials, onClose, schema }) => {
  ...
  // Colunas que servem como identificadores de fallback
  const getIdentifierColumns = () => {
    if (!schema) return ['id', 'codigo', 'cod', 'grid'];
    ...
    if (!table) return ['id', 'codigo', 'cod', 'grid'];
  ...
  const fetchData = async (column: string, value: any, isFallback = false) => {
    ...
    try {
      ...
      if (!isFallback) {
        // Busca Primária
        const sql = `SELECT * FROM ${targetTable} WHERE ${column} = '${safeValue}' LIMIT 50`;
        ...
        if (res.length === 1) {
          ...
          // 0 resultados: Tentar Fallback
          await performFallbackSearch(value);
        }
      } else {
        // Busca de um item específico selecionado na lista de ambiguidade
        const sql = `SELECT * FROM ${targetTable} WHERE ${column} = '${safeValue}' LIMIT 1`;
        const res = await executeQueryReal(credentials, sql);
        setData(res);
        setViewMode('cards');
        setActiveSearchCol(column);
  ...
  const performFallbackSearch = async (value: any) => {
    ...
    // Construir query que busca em todas as colunas candidatas
    ...
    try {
      const res = await executeQueryReal(credentials, sql);
      if (res.length === 0) {
        setData([]); // Realmente não encontrado
      } else if (res.length === 1) {
        setData(res);
        // Tenta descobrir qual coluna deu match para o label
      ...
      setData([]); // Fallback falhou silenciosamente ou por tipos incompatíveis
    }
  };
  ...
  const selectRow = (row: any) => {
    // Ao selecionar da lista, usamos a PK ou o primeiro identificador encontrado para garantir o foco
    const idCol = getIdentifierColumns().find(c => row[c] !== undefined) || columns[0];
    setData([row]);
    setActiveSearchCol(idCol);
    setViewMode('cards');
  };
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-5xl h-[90vh] rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex flex-col sm:flex-row justify-between items-center gap-4 shrink-0">
           <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                 ...
                 <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-bold">Inspeção de Dados</p>
              </div>
           </div>
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50 dark:bg-slate-950 relative custom-scrollbar">
           {loading ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400 bg-white/50 dark:bg-slate-900/50 z-20">
                 ...
                 <span className="text-sm font-medium">Buscando registros...</span>
              </div>
           ) : error ? (
              ...
              /* DESAMBIGUAÇÃO: Múltiplos registros encontrados */
              <div className="p-8 max-w-3xl mx-auto">
                 <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-2xl p-6 mb-6 flex items-start gap-4">
                    ...
                    <div>
                       ...
                       <p className="text-indigo-700 dark:text-indigo-300 text-sm mt-1">
                          A busca por <strong>"{filterValue}"</strong> retornou {ambiguousRows.length} resultados em diferentes colunas (id, código, grid, etc). 
                          Por favor, selecione qual você deseja detalhar:
                       </p>
                    </div>
                 </div>
                 ...
                 <div className="space-y-3">
                    {ambiguousRows.map((row, idx) => {
                       const rowId = row['id'] || row['grid'] || row['codigo'] || idx;
                       const descCol = columns.find(c => c.toLowerCase().includes('nome') || c.toLowerCase().includes('desc')) || columns[1];
                             ...
                             <div className="flex items-center gap-4">
                                ...
                                <div>
                                   <div className="font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                                      {String(row[descCol] || 'Registro sem descrição')}
                                   </div>
                                   <div className="flex gap-3 mt-1">
                                      {getIdentifierColumns().filter(c => row[c] !== undefined).slice(0, 3).map(c => (
                                         <span key={c} className="text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded font-mono border border-slate-200 dark:border-slate-700">
                                            {c}: {row[c]}
                                         </span>
                                      ))}
                                   </div>
              ...
              <div className="p-20 text-center text-slate-400 flex flex-col items-center">
                 ...
                 <p className="text-sm opacity-60">Tentamos buscar por grid, id e códigos, mas não houve correspondência.</p>
              </div>
           ) : (
        ...
        <div className="p-4 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center shrink-0">
           <div className="flex items-center gap-2 text-xs text-slate-400">
              ...
              <span>Busca inteligente ativa (grid, id, codigo)</span>
           </div>
           <button 
              onClick={onClose}
              className="px-8 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-bold shadow-lg transition-all"
           >
              Fechar
           </button>
        </div>
