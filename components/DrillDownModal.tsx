...
import { X, Database, ArrowRight, Loader2, AlertCircle, LayoutGrid, Search, Copy, Check, MousePointer2, ChevronRight, Table2, HelpCircle, Save, Key, AlertTriangle, ChevronDown, ChevronUp, Eye } from 'lucide-react';
...
const DrillDownModal: React.FC<DrillDownModalProps> = ({ targetTable, filterColumn, filterValue, credentials, onClose, schema }) => {
  ...
  const toggleExpandField = (id: string) => {
  ...
  const fetchData = async (column?: string, value?: any, isManual = false) => {
    ...
    try {
      ...
      if (results.length === 0) {
        if (isManual) setError(`O valor "${val}" não foi encontrado na coluna "${column}".`);
        else setViewMode('map_column');
        setLoading(false);
        return;
      }
      ...
      if (matchedColumnsSet.size > 1 && !isManual) {
        ...
        // NOVIDADE: Auto-switch baseado na contagem
        if (results.length === 1) {
          setViewMode('cards');
        } else {
          setViewMode('table');
  ...
  useEffect(() => {
    const mappings = getUserMappings();
    if (mappings[targetTable]) fetchData(mappings[targetTable], filterValue, true);
    else fetchData();
    ...
    <div className="fixed inset-0 z-[110] bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white dark:bg-slate-800 w-full max-w-5xl h-[90vh] rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
           ...
           {loading ? (
              ...
              <div className="p-8 max-w-4xl mx-auto">
                 <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800 rounded-2xl p-6 mb-8 flex items-start gap-4 shadow-sm">
                    ...
                    <div>
                       <h4 className="font-bold text-amber-900 dark:text-amber-100 text-lg">Múltiplas correspondências</h4>
                       <p className="text-amber-700 dark:text-amber-300 text-sm mt-1">
                          O valor foi encontrado em diferentes campos. Selecione o registro desejado:
                       </p>
                    </div>
                 </div>
                 <div className="grid grid-cols-1 gap-3">
                    {ambiguousMatches.map((match, idx) => {
                       ...
                       return (
                          <button key={idx} onClick={() => { setData([row]); setActiveSearchCol(match.matchedColumn); setViewMode('cards'); }} className="w-full flex items-center justify-between p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl hover:border-indigo-500 hover:shadow-lg transition-all group text-left">
                             <div className="flex items-center gap-5">
                                <div className="w-10 h-10 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center text-slate-400 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors"><MousePointer2 className="w-5 h-5" /></div>
                                <div className="min-w-0">
                                   <div className="text-[10px] font-bold bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-300 px-2 py-0.5 rounded w-fit mb-1 border border-indigo-100 dark:border-indigo-800">Coluna: {match.matchedColumn}</div>
                                   <div className="font-bold text-slate-800 dark:text-slate-100 text-base truncate">{String(row[descCol] || 'Sem descrição')}</div>
                                </div>
                             </div>
              ...
              <div className="p-8 max-w-4xl mx-auto">
                 <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-6 mb-8 flex items-start gap-5 shadow-sm">
                    <div className="p-4 bg-white dark:bg-slate-800 rounded-2xl shadow-sm text-amber-500 shrink-0"><HelpCircle className="w-8 h-8" /></div>
                    <div><h4 className="font-bold text-amber-900 dark:text-amber-100 text-xl">Busca sem resultados</h4><p className="text-amber-700 dark:text-amber-300 text-sm mt-1">O valor não foi encontrado nos campos padrão. Selecione a coluna correta:</p></div>
                 </div>
                 <div className="mb-6 relative">
                    <Search className="absolute left-4 top-3.5 w-4 h-4 text-slate-400" />
                    <input type="text" placeholder="Filtrar colunas..." value={columnFilterTerm} onChange={e => setColumnFilterTerm(e.target.value)} className="w-full pl-11 pr-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500 shadow-sm" />
                 </div>
                 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {filteredTargetColumns.map((col) => (
                       <button key={col.name} onClick={() => fetchData(col.name, filterValue, true)} className="flex items-center gap-3 p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-indigo-500 hover:shadow-md transition-all group text-left">
                          <div className="w-9 h-9 bg-slate-50 dark:bg-slate-800 rounded-lg flex items-center justify-center text-slate-400 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors">{col.isPrimaryKey ? <Key className="w-4 h-4 text-amber-500" /> : <Table2 className="w-4 h-4" />}</div>
                          <div className="min-w-0"><div className="font-bold text-slate-800 dark:text-slate-100 text-sm truncate">{col.name}</div><div className="text-[10px] text-slate-400 font-mono uppercase">{col.type}</div></div>
                       </button>
                    ))}
                 </div>
              </div>
                 ...
                 {viewMode === 'table' ? (
                    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-x-auto custom-scrollbar">
                       <table className="w-full text-left text-sm border-collapse min-w-max">
                          <thead className="bg-slate-50 dark:bg-slate-800 sticky top-0 shadow-sm z-10 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                             <tr>{columns.map(col => <th key={col} className="px-4 py-3 border-b border-slate-200 dark:border-slate-700">{col}</th>)}</tr>
                          </thead>
                          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                             {filteredData.map((row, i) => (
                                <tr key={i} className="hover:bg-indigo-50/30 dark:hover:bg-indigo-900/10 transition-colors group">
                                   {columns.map(col => (
                                      <td key={col} className={`px-4 py-3 text-xs font-mono border-b border-transparent ${col === activeSearchCol ? 'text-indigo-600 dark:text-indigo-400 font-bold bg-indigo-50/20' : 'text-slate-600 dark:text-slate-400'}`}>
                    ...
                    <div className="space-y-6">
                       {filteredData.map((row, rowIndex) => (
                          <div key={rowIndex} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-2">
                             <div className="px-4 py-2 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2"><Database className="w-3 h-3" /> Registro #{rowIndex + 1}</span>
                             ...
                             <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-0.5">
                                {columns.map(col => {
                                   ...
                                   return (
                                      <div key={col} className={`group flex flex-col py-1.5 px-2 rounded transition-all border-b border-slate-50 dark:border-slate-800/40 last:border-0 ${isMatch ? 'bg-yellow-50 dark:bg-yellow-900/20' : 'hover:bg-slate-50 dark:hover:bg-slate-800/30'}`}>
                                         <div className="flex items-center justify-between">
                                            <span className={`text-[11px] font-bold uppercase truncate pr-4 max-w-[140px] ${col === activeSearchCol ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500'}`} title={col}>{col}</span>
                                            <div className="flex items-center gap-2">
                                               {isLong && (
                                                  <button onClick={() => toggleExpandField(fieldId)} className="text-[10px] font-bold text-indigo-500 hover:text-indigo-700 flex items-center gap-0.5 transition-colors">
                                                     {isExpanded ? <><ChevronUp className="w-3 h-3" /> Ver Menos</> : <><ChevronDown className="w-3 h-3" /> Ver Tudo</>}
                                                  </button>
                                               )}
                                               <button onClick={() => handleCopy(val, fieldId)} className="opacity-0 group-hover:opacity-100 p-1 text-slate-400 hover:text-indigo-600 transition-all"><Copy className="w-3 h-3" /></button>
                                            </div>
                                         </div>
                                         <div className={`text-xs font-mono transition-all ${isExpanded ? 'whitespace-pre-wrap break-words mt-1' : 'truncate'} ${val === null ? 'text-slate-300 italic' : col === activeSearchCol ? 'text-indigo-700 dark:text-indigo-300 font-bold' : 'text-slate-700 dark:text-slate-300'}`}>
                                            {valStr}
                                         </div>
                                      </div>
                                   );
        ...
        <div className="p-4 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center shrink-0">
           <div className="flex items-center gap-2 text-xs text-slate-400"><Save className="w-3.5 h-3.5" /><span>Memória ativa para <strong>{targetTable}</strong></span></div>
