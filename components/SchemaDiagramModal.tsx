...
const CanvasMinimap = memo(({ 
   ...
   useEffect(() => {
      ...
      Object.entries(positions).forEach(([tableName, pos]) => {
         ...
         const h = (HEADER_HEIGHT + 20) * mapScale; // Approximate height
...
const DiagramNode = memo(({
  ...
  // High performance style application
...
const SchemaDiagramModal: React.FC<SchemaDiagramModalProps> = ({ schema, onClose }) => {
  ...
  // --- Layout Calculation (Optimized grid) ---
  ...
  useEffect(() => {
    ...
    // Defer layout calculation to next tick to avoid blocking UI
    setTimeout(() => {
  ...
  const { visibleTables, bounds } = useMemo(() => {
     ...
     // Add padding for size of tables
     maxX += TABLE_WIDTH;
     maxY += 500;

     // 2. Viewport Calculation (Visible Rect in World Coordinates)
  ...
  const handleWheel = (e: React.WheelEvent) => {
    ...
    // Simple zoom to center could be improved to zoom to mouse
    setScale(newScale);
  };
  ...
  // --- SVG Connections Generation (Optimized) ---
  const connections = useMemo(() => {
    // Aggressive Culling: If interacting or zoomed out, hide connections unless specifically focused
    if ((isInteracting && visibleTables.length > 30) || scale < 0.4) {
       if (!hoveredNode && !selectedRelationship && !pathMode) return [];
    }

    const lines: React.ReactElement[] = [];
    const visibleSet = new Set(visibleTables.map(t => t.name));
    
    // Determine which tables we need to draw lines for
    // Priority: Path > Selection > Hover > Visible Viewport
    ...
    if (pathMode && foundPath.length > 0) {
       ...
       // Only draw connections related to hovered node
    ...
    sourceTables.forEach(table => {
      ...
      table.columns.forEach((col, colIndex) => {
        if (col.isForeignKey && col.references) {
          ...
          // Visibility Check: Don't draw lines to off-screen nodes unless they are part of active focus
          ...
          if (pathMode) {
             ...
             // If hovering a node, only show lines connected to it
             if (table.name !== hoveredNode && targetTable !== hoveredNode) return;
          }

          // Calculate Anchor Points
          // Simple heuristic for speed: center of row vs top of table
          const sourceY = startPos.y + HEADER_HEIGHT + (colIndex * ROW_HEIGHT) + (ROW_HEIGHT / 2);
          let targetY = endPos.y + 20; // Default to top
          
          // More precise target calculation only if focused/high LOD
          ...
          if (isPathLine) { stroke = "#06b6d4"; width = 4; opacity = 1; }
          else if (isHighlighted) { stroke = "#f59e0b"; width = 3; opacity = 1; }
          else if (hoveredNode) { stroke = "#6366f1"; width = 2; opacity = 0.8; }

          lines.push(
             <path 
                key={`${table.name}-${col.name}-${targetTable}`}
                d={pathD}
                stroke={stroke}
                strokeWidth={width}
                fill="none"
    ...
    return lines;
  }, [visibleTables, positions, lodLevel, isInteracting, hoveredNode, selectedRelationship, pathMode, foundPath, schema.tables, relationshipGraph]);
  ...
  const handleExportImage = async () => {
     ...
     try {
        const canvas = await html2canvas(containerRef.current, {
           ...
           ignoreElements: (el) => el.classList.contains('minimap-ignore')
        });
        const link = document.createElement('a');
      ...
      <div className="bg-slate-100 dark:bg-slate-900 w-full h-full rounded-xl shadow-2xl overflow-hidden relative border border-slate-700 flex flex-col">
        ...
        <div className="absolute top-4 left-4 z-10 flex flex-col gap-2 pointer-events-none minimap-ignore">
           ...
           <div className="bg-white dark:bg-slate-800 p-2 rounded-lg shadow-md border border-slate-200 dark:border-slate-700 pointer-events-auto w-64">
              <button 
                 onClick={() => { setPathMode(!pathMode); setFoundPath([]); setPathStartNode(null); setPathEndNode(null); }}
                 className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold w-full transition-colors ${pathMode ? 'bg-cyan-100 text-cyan-800' : 'bg-slate-100 text-slate-600'}`}
              >
                 <Route className="w-4 h-4" /> {pathMode ? 'Modo Rota Ativo' : 'Buscar Caminho'}
              </button>
              {pathMode && (
                 <div className="mt-2 text-[10px] text-slate-500 text-center">
                    Clique em duas tabelas para achar a rota.
                 </div>
              )}
           </div>
        </div>
        ...
        >
          ...
          >
             ...
             {/* Render Visible Nodes Only (Virtualization) */}
        ...
        {contextMenu && (
           <div style={{ top: contextMenu.y, left: contextMenu.x }} className="fixed z-[80] bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 py-2 min-w-[150px] animate-in zoom-in-95 duration-100" onClick={e => e.stopPropagation()}>
              <button onClick={() => { 
                 const p = positions[contextMenu.tableName];
                 setPan({ x: (containerSize.w/2) - (p.x * 1.5) - 100, y: (containerSize.h/2) - (p.y * 1.5) }); 
                 setScale(1.5); 
                 setContextMenu(null); 
              }} className="w-full text-left px-3 py-1.5 text-xs hover:bg-slate-100 dark:hover:bg-slate-700 dark:text-slate-200">Focar Tabela</button>
              ...
              <div className="px-3 flex gap-1 flex-wrap">
                 {TABLE_COLORS.map(c => (
                    <button key={c.id} onClick={() => {
                       setTableColors(prev => ({ ...prev, [contextMenu.tableName]: c.id }));
                       setContextMenu(null);
                    }} className={`w-4 h-4 rounded-full ${c.bg.replace('50', '400')}`} />
                 ))}
              </div>
           </div>
        )}
        ...
        {/* Canvas Minimap (High Performance) */}
